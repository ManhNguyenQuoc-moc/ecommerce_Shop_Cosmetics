...
export default function NewsletterSection() {
  return (
    <div className="relative overflow-hidden min-h-58 bg-linear-to-br from-brand-500 to-brand-700 rounded-2xl p-6 sm:p-8 text-white shadow-lg shadow-brand-200/50 border border-brand-400/30">
      {/* Hiệu ứng quét sáng giống HeroBanner */}
      <div className="absolute inset-0 z-1 bg-white/30 animate-sweep pointer-events-none mix-blend-overlay" />
      ...
      <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6 md:gap-8">
        ...
        <div className="w-full md:w-115 space-y-2">
          <div className="flex flex-col sm:flex-row gap-2">
            <div className="flex-1">
              <input
                type="email"
                placeholder="Nhập email của bạn..."
                className="bg-white/95 backdrop-blur-sm rounded-xl border-none h-12 px-4 text-sm text-gray-900 w-full shadow-inner focus:ring-2 focus:ring-brand-200 transition-all placeholder:text-gray-400"
              />
            </div>
            <button
              type="button"
              className="h-12 w-full sm:w-auto rounded-xl bg-white text-brand-500 border-none text-sm font-bold flex items-center justify-center gap-2 shadow-lg px-5"
            >
              <Send size={16} />
              Đăng ký
            </button>
