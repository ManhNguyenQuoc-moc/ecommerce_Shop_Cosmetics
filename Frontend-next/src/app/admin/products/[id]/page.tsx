...
export default function ProductDetailPage({ params }: { params: { id: string } }) {
  ...
  const totalRevenue = product.variants?.reduce((sum: number, v: any) => sum + (v.soldCount * (v.salePrice || v.price)), 0) || 0;
    ...
    <div className="space-y-6 animate-fade-in relative z-0">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Button type="primary" size="large" icon={<PencilLine size={18} />}>
            Sửa Sản Phẩm
          </Button>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="flex flex-col md:flex-row gap-6">
              ...
              <div className="w-full md:w-2/3 flex flex-col justify-center">
                <div className="inline-flex items-center gap-2 mb-2">
                  <span className="bg-brand-100 text-brand-700 text-xs font-bold px-2.5 py-1 rounded-md">{product.category}</span>
                  <span className="bg-slate-100 text-slate-600 text-xs font-bold px-2.5 py-1 rounded-md">{product.brand}</span>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-4 mb-4">
                  ...
                  <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400 border-l border-slate-200 dark:border-slate-700 pl-4">
                    ...
                    <span className="text-sm font-semibold">{product.commentCount} bình luận</span>
                  </div>
                </div>
          ...
          <div className="bg-white/90 dark:bg-slate-900/80 backdrop-blur-md rounded-3xl p-6 shadow-sm border border-slate-200 dark:border-pink-500/20">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h4 className="text-xl font-bold text-slate-800 dark:text-white">Các Phiên Bản Gốc (Variants)</h4>
            </div>

            <div className="space-y-3">
              {product.variants?.map((v: any) => (
                <Link href={`/admin/variants/${v.id}`} key={v.id}>
                  <div className="flex items-center justify-between p-4 rounded-2xl border border-slate-100 dark:border-slate-800 hover:border-fuchsia-300 dark:hover:border-fuchsia-500/40 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all cursor-pointer group">
                    <div>
                      <h5 className="font-bold text-slate-800 dark:text-slate-200 group-hover:text-fuchsia-600 transition-colors">
                        {[v.size, v.color].filter(Boolean).join(" - ") || "Bản chuẩn"}
                      </h5>
                      <p className="text-sm font-semibold text-slate-500">Mã SKU: <span className="text-slate-700 dark:text-slate-400">{v.sku}</span></p>
                    </div>
                    <div className="flex items-center gap-6 text-right">
                      ...
                      <div className="min-w-[80px]">
                        ...
                        <p className={`font-bold ${v.stock > 0 ? 'text-emerald-600' : 'text-red-500'}`}>{v.stock}</p>
                      </div>
                    </div>
                  </div>
                </Link>
            ...
            <div className="space-y-6 mt-6">
              <div className="flex justify-between items-end">
                <div>
                  ...
                  <p className="text-3xl font-black text-slate-800 dark:text-white">{product.sold}</p>
                </div>
              ...
              <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
                 ...
                 <div className="space-y-4">
                    {product.variants?.map((v: any, i: number) => {
                      const pct = product.sold > 0 ? Math.round((v.soldCount / product.sold) * 100) : 0;
                      return (
                      <div key={v.id}>
