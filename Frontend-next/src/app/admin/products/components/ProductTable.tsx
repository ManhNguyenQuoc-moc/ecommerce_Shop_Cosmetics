...
export default function ProductTable({ isPending }: ProductTableProps) {
  ...
  {record.image ? (
    ...
    },
    {
      ...
      dataIndex: "category",
      key: "category",
      render: (category: any) => <div className="text-text-sub font-medium text-sm">{category?.name || "Chưa phân loại"}</div>,
    },
    {
      title: "Thương hiệu",
      dataIndex: "brand",
      key: "brand",
      render: (brand: any) => <div className="font-semibold text-text-sub text-sm">{brand?.name || "N/A"}</div>,
    },
    {
      title: "Tổng đã bán",
      dataIndex: "sold",
  ...
  return (
    <div className="w-full">
      ...
      {selectedRowKeys.length > 0 && (
        <div className="flex items-center gap-3 mt-4 px-4 py-3 bg-brand-50/50 dark:bg-brand-500/5 border border-brand-100 dark:border-border-brand rounded-xl">
          <span className="text-sm font-semibold text-brand-700 dark:text-brand-500 flex-1">
          ...
          <SWTTooltip title={isHiddenTab ? "Khôi phục các sản phẩm đã chọn" : "Ẩn các sản phẩm đã chọn"} color={isHiddenTab ? "#10b981" : "#f59e0b"} placement="top">
            <button
              ...
              className={`flex items-center gap-1.5 h-8 px-3 text-xs font-semibold rounded-lg ${isHiddenTab ? "bg-status-success-bg hover:opacity-80 text-status-success-text border-status-success-border" : "bg-status-warning-bg hover:opacity-80 text-status-warning-text border-status-warning-border"} transition-colors cursor-pointer border`}
            >
          ...
          <SWTTooltip title="Bỏ chọn tất cả" placement="top">
            <button
              ...
              className="flex items-center gap-1.5 h-8 px-3 text-xs font-semibold rounded-lg bg-bg-muted hover:opacity-80 text-text-sub border border-border-default transition-colors cursor-pointer"
            >
              Bỏ chọn
            </button>
          </SWTTooltip>
