...
import { CreateRoleDto, useCreateRole, RBAC_API_ENDPOINT } from "@/src/services/admin/rbac";
...
export default function AddRoleModal({
  ...
  const { trigger: createRole, isMutating: creating } = useCreateRole();
  ...
  const handleSubmit = async () => {
    try {
      ...
      showNotificationSuccess("Role mới đã được tạo thành công!");
      form.resetFields();
      onClose();
      onSuccess?.();
    } catch (error) {
      ...
      if (!(error instanceof Error && error.name === "ValidationError")) {
        showNotificationError("Đã có lỗi xảy ra khi tạo role. Vui lòng thử lại.");
      }
    }
  };
