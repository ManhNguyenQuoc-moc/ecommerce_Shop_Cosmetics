...
export interface IDiscountService {
  getAllVouchers(userId?: string, skip?: number, take?: number, filters?: VoucherQueryFiltersDTO): Promise<{ items: VoucherResponseDTO[], total: number }>;
  getActiveVouchers(userId?: string, skip?: number, take?: number, filters?: VoucherQueryFiltersDTO): Promise<{ items: VoucherResponseDTO[], total: number }>;
